...
class Fingerprint():
    ...
    def get_template(self, idx):
        """
        Récupère le modèle d'empreinte digitale pour l'ID spécifié.

        :param idx: ID de l'empreinte digitale à récupérer
        :return: Le modèle d'empreinte digitale en bytes, ou None si une erreur s'est produite
        """
        # Envoyer la commande GetTemplate avec l'ID de l'empreinte
        if self._send_packet("GetTemplate", idx):
            # Lire la réponse du paquet
            ack, param, res, data = self._read_packet()
            if not ack:
                return None, False
            return data, True if param == 0 else False
    ...
    def change_baudrate(self, baud_rate):
        """
        Change le baud rate de l'appareil.

        :param baud_rate: Nouveau baud rate à définir (ex: 115200)
        :return: True si le changement a réussi, False sinon
        """
        # Envoyer la commande ChangeBaudrate avec le baud rate souhaité
        if self._send_packet("ChangeBaudrate", baud_rate):
            # Lire la réponse du paquet
            ack, param, res, data = self._read_packet()
            if not ack:
                return False
